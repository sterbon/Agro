...
class [[eosio::contract]] foodscm : public eosio::contract {
  private:
    ...
    struct [[eosio::table]] cropdetails {
        ...
        string uploadDate;
        string soldDate;
        vector<string> trackingLocation;
        vector<string> trackingDate;
  ...
  public:
    ...
    void uploadcrop(name producer, string cropName, uint64_t cropAmount, string imageHash, 
      ...
      _cropdata.emplace(_self, [&](auto& row) {
          ...
          row.trackingLocation.emplace_back(home);
	        row.trackingDate.emplace_back(uploadDate);
      }); 
    }
    
 [[eosio::action]]
    void updateloc(name buyer, uint64_t cropPid, string updatedLoc, string updatedDate)
    {
      ...
      auto payer = has_auth( seller ) ? seller : buyer;
         _cropdata.modify(itr, payer, [&](auto& row){
          ...
          row.trackingLocation.emplace_back(updatedLoc);
	        row.trackingDate.emplace_back(updatedDate);
