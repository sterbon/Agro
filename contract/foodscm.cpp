...
private:
  ...
  typedef eosio::multi_index<name("agrotable3"), cropdetails> crop_data;
...
public:
  ...
  [[eosio::action]] void userdata(name customer, string accType, uint64_t tokenBalance)
  {
  ...
  [[eosio::action]] void uploadcrop(name producer, string cropName, uint64_t cropAmount, string imageHash,
                                    uint64_t price, string dateOfHarvest, string dateOfSow, string fertilizers,
                                    string uploadDate, string home)
  {
    ...
    _cropdata.emplace(_self, [&](auto &row) {
      ...
      row.tracking_details.emplace_back(home);
    });
  }

  [[eosio::action]] void updateloc(name buyer, uint64_t cropPid, string location)
  {
    ...
    _cropdata.modify(itr, payer, [&](auto &row) {
      row.tracking_details.emplace_back(location);
    });
  }
  [[eosio::action]] void buycrop(name buyer, uint64_t cropPid, asset price, string memo)
  {
    auto itr = _cropdata.find(cropPid);
    name seller = itr->producer;
