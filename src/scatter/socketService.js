...
export default class SocketService {
        ...
        return new Promise(async resolve => {
	        ...
	        const setupSocket = () => {
		        ...
		        const event_api = ({event, payload}) => {
                    ...
                    return Object.keys(this.eventHandlers).map(key => {
	...
	static getOriginOrPlugin(plugin){
        let origin;
        //Saste haxx used below. Please remove window. from window.location if giving errors
		if(typeof window.location !== 'undefined')
			if(window.location.hasOwnProperty('hostname') && window.location.hostname.length && window.location.hostname !== 'localhost')
